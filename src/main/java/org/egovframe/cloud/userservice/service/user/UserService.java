...
public class UserService extends AbstractService implements UserDetailsService {
    ...
    /**
     * 구글 클라이언트 ID commented due to oauth2 error
     */
    //@Value("${spring.security.oauth2.client.registration.google.client-id}")
    //private String GOOGLE_CLIENT_ID;

    /**
     * 카카오 사용자 정보 URL commented due to oauth2 error
     */
    //@Value("${spring.security.oauth2.client.provider.kakao.user-info-uri}")
    private String KAKAO_USER_INFO_URI = "HelloDummy";

    /**
     * 네이버 사용자 정보 URL commented due to oauth2 error
     */
    //@Value("${spring.security.oauth2.client.provider.naver.user-info-uri}")
    private String NAVER_USER_INFO_URI = "HelloDummy";
    ...
    private SocialUserResponseDto getGoogleUserInfo(String token) {
        try {
            ...
            GoogleIdTokenVerifier verifier = new GoogleIdTokenVerifier.Builder(transport, gsonFactory)
                    //.setAudience(Collections.singletonList(GOOGLE_CLIENT_ID)) comment due to oauth2 error
                    .build();
    ...
    private UserResponseDto getAndSaveSocialUser(String providerCode, String providerId, String email, String userName) {
        ...
        // 현재 시스템 구조 상 이메일을 사용자 식별키로 사용하고 있어서 이메일로 사용자를 한번 더 검색한다.HUserApiController
        if (user == null) {
